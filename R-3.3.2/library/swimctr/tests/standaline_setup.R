...
setwd("D:/Projects/Clients/OR_DOT/SWIM_WOC7/Models/SupportingPackages/tests/")
...
this_year <- 2018
faf_data <- readr::read_csv(file.path(DATA_RAW, "faf42_oregon.csv.gz"))
truck_allocation_factors <- readr::read_csv(file.path(DATA_RAW, "faf35-truck-allocation-factors.csv"))
truck_equivalency_factors <- readr::read_csv(file.path(DATA_RAW, "faf35-truck-equivalency-factors.csv"))
empty_truck_factors <- readr::read_csv(file.path(DATA_RAW, "faf35-empty-truck-factors.csv"))
...
intermodal_connectors <- readr::read_csv(file.path(DATA_RAW, "ct-intermodal-connectors.csv"))
external_gateways <- readr::read_csv(file.path(DATA_RAW, "ct-external-station-equivalencies.csv"))
save_to <- "daily_faf_trips.csv"

# create_makeuse_coefficients
pecas_makeuse <- readr::read_csv("../tests/MakeUse.csv")
faf_data <- readr::read_csv(file.path(DATA_RAW, "faf42-oregon.csv.gz"))
...
utility_parameters <- readr::read_csv(file.path(DATA_RAW, "ct-destination-utility-parameters.csv"))
trip_length_targets <- readr::read_csv(file.path(DATA_RAW, "ct-trip-length-targets.csv"))
