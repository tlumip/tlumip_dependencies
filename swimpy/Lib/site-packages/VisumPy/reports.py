...
def turnValueByOrientation(Visum, attribute):
    ...
    while nodeIter.Valid:
        ...
        if node.Active:
            ...
            resultRow = numpackage.repeat([0.0],numTurnsPerNode + 2) #node no and 4leg check extra

            if numTurns > 0:

                #Get orientation field
                if int(Visum.VersionNumber[0:2]) >= 11:
                    orientation = numpackage.array(GetMulti(turns,"OrientationHBS"))
                else:
                    orientation = numpackage.array(GetMulti(turns,"Orientation"))
                    
                orientation = orientation.astype(numpackage.int)
                
                #Get attribute value
                attributeValue = numpackage.array(GetMulti(turns,attribute))
    ...
    #Return result
    return [numpackage.transpose(result),attributesList]
