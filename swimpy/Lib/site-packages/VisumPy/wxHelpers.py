...
class wxNetobjCombo(wx.ComboBox):
    ...
    def InitChoices(self, container, displayattrs, resultattr = None, filtervector = None, displayformatter = None, key = None, matrixType = "DATA"):
        ...
        if filtervector != None: # keep only the tuples for which filtervector == True
            try:
                filterHelp = filtervector[0][0]
            ...
            if len(filtervector_internal) != len(dispvals):
                raise "wxNetobjCombo.InitChoices: length of filtervector does not match number of objects in container"
            activedispvals = []
        ...
        if key == None:
            ...
            if filtervector_internal != None: # keep only the key values for which filtervector_internal == True
                activeKeys = []
                for i in xrange(len(filtervector_internal)):
                    if filtervector_internal[i]:
        ...
        if filtervector_internal != None: # keep only the result values for which filtervector_internal == True
            activeresultvals = []
            for i in xrange(len(filtervector_internal)):
                if filtervector_internal[i]:
