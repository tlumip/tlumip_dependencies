...
def __CopyFromNumpy(VisumMat, NumpyMat):
    VisumMatShape = __GetVisumMatrixShape(VisumMat)
...
def GetMatrixRaw(Visum, which, intoMat = None):
    ...
    CAUTION: This function only works if the script is executed within VISUM
    (from the script menu or within Calculate - Procedures). In scripts executed
    outside VISUM, it will crash VISUM!
